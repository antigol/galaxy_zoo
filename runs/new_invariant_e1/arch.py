...
class CNN:
    ...
    def NN(self, x):
        ...
        xi, xr = nn.convolution2(x, None, 4, 2, w=4, s=2) # 211
        ...
        assert xr.get_shape().as_list() == [None, 209, 209, 8*2]
        xi, xr = nn.convolution2(xi, xr, 8, 4, w=5, s=2) # 103
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        ...
        assert xr.get_shape().as_list() == [None, 101, 101, 8*4]
        xi, xr = nn.convolution2(xi, xr, 16, 8, w=5, s=2) # 49
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        ...
        assert xr.get_shape().as_list() == [None, 47, 47, 8*8]
        xi, xr = nn.convolution2(xi, xr, 32, 16, w=5, s=2) # 22
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        ...
        assert xr.get_shape().as_list() == [None, 20, 20, 8*16]
        xi, xr = nn.convolution2(xi, xr, 64, 32, w=4, s=2) # 9
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)

        ########################################################################
        assert xi.get_shape().as_list() == [None, 9, 9, 64]
        assert xr.get_shape().as_list() == [None, 9, 9, 8*32]
        xi, xr = nn.convolution2(xi, xr, 128, 64) # 7
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
