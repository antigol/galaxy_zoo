...
class CNN:
    ...
    def NN(self, x):
        ...
        xi, xr = nn.convolution2(x, None, 8, 1, w=4, s=2) # 211
        ...
        ########################################################################
        assert xi.get_shape().as_list() == [None, 209, 209, 8]
        assert xr.get_shape().as_list() == [None, 209, 209, 8*1]
        xi, xr = nn.convolution2(xi, xr, 16, 2, w=5, s=2) # 103
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        xi, xr = nn.convolution2(xi, xr) # 101

        ########################################################################
        assert xi.get_shape().as_list() == [None, 101, 101, 16]
        assert xr.get_shape().as_list() == [None, 101, 101, 8*2]
        xi, xr = nn.convolution2(xi, xr, 32, 4, w=5, s=2) # 49
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        xi, xr = nn.convolution2(xi, xr) # 47

        ########################################################################
        assert xi.get_shape().as_list() == [None, 47, 47, 32]
        assert xr.get_shape().as_list() == [None, 47, 47, 8*4]
        xi, xr = nn.convolution2(xi, xr, 64, 8, w=5, s=2) # 22
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        xi, xr = nn.convolution2(xi, xr) # 20

        ########################################################################
        assert xi.get_shape().as_list() == [None, 20, 20, 64]
        assert xr.get_shape().as_list() == [None, 20, 20, 8*8]
        xi, xr = nn.convolution2(xi, xr, 128, 16, w=4, s=2) # 9
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)

        ########################################################################
        assert xi.get_shape().as_list() == [None, 9, 9, 128]
        assert xr.get_shape().as_list() == [None, 9, 9, 8*16]
        xi, xr = nn.convolution2(xi, xr, 256, 32) # 7
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        ...
        ########################################################################
        assert xi.get_shape().as_list() == [None, 1, 1, 256]
        ...
        xi, xr = nn.fullyconnected2(xi, xr, 512, 64)
        xi = nn.batch_normalization(xi, self.tfacc, input_repr='invariant')
        xr = nn.batch_normalization(xr, self.tfacc)
        xi = tf.nn.dropout(xi, self.tfkp)
        xr = tf.nn.dropout(xr, self.tfkp)
